...
BestInPlaceEditor.forms = {
  ...
  "ckeditor" : {
    activateForm : function() {
	  ...
	  this.tid = $(this.element).data('object') + '_' + $(this.element).data('attribute')
      // construct the form
      var output = '<form action="javascript:void(0)" style="display:inline;"><textarea id="' + this.tid + '">';
      output += this.sanitizeValue(this.display_value);
      output += '</textarea>';
      if (this.okButton) {
        output += '<input type="submit" value="' + this.okButton + '" />'
      ...
      this.element.find("textarea")[0].focus();
      this.element.find("form").live('submit', {editor: this}, BestInPlaceEditor.forms.textarea.submitHandler);
      if (this.cancelButton) {
        this.element.find("input[type='button']").bind('click', {editor: this}, BestInPlaceEditor.forms.textarea.cancelButtonHandler)
      }
      //this.element.find("textarea").bind('blur', {editor: this}, BestInPlaceEditor.forms.textarea.blurHandler);
	  ...
	  var lang = $(this.element).data('lang')
	  var toolbar = $(this.element).data('toolbar')
	  
	  console.log(toolbar)
	  
	  if (CKEDITOR.instances[this.tid]) {
		  CKEDITOR.remove(CKEDITOR.instances[this.tid]);
	  }
	  
	  CKEDITOR.replace(this.tid, { "height": height, "width": width, "language": lang,"toolbar": 'Full' });
    ...
    getValue :  function() {
	    var ckeditor = CKEDITOR.instances[this.tid]
